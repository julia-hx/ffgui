...
//TODO: global args object? run all commands from here
//pub fn run_command(input_file_path: &Option<PathBuf>, args);
//pub fn run_command_bach(input_file_paths: Vec<&Option<PathBuf>>, args);

pub fn to_mp3(input_file_path: &Option<PathBuf>, quality: Quality) {
	let input_path = input_file_path.clone();
	...
	thread::spawn(move || {
		...
		// to string
		let path_string = input_path.expect("no valid path").into_os_string().into_string().unwrap();
